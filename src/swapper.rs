...
impl<'a> Swapper<'a> {
  ...
  pub fn execute_thumbs(&mut self) {
    ...
    let pane_command = format!(
        "tmux capture-pane -e -J -t {active_pane_id} -p{scroll_params} | tail -n {height} | {dir}/target/release/thumbs -f '%U:%H' -t {tmp} {args}; tmux swap-pane -t {active_pane_id}; {zoom_command} tmux wait-for -S {signal}",
